export default {    
    en: 'English',
    es: 'Español',
    fr: 'Français',
    kr: '한국어',
    zh_hans: '汉语',
    zh_hant: '漢語',        
    myProfile: 'My Profile',
    schoolProfile: 'School Profile',
    logout: 'Logout',        
    assign: 'Assign',
    school: 'School',
    name: 'Name',
    course: 'Course',
    teacher: 'Teacher',
    studentsCount: 'Students Count',
    add: 'Add',
    addNew: 'Add New',
    classroomsNotFound: 'Classes Not Found...',
    homeroomsNotFound: 'Homerooms Not Found...',
    classrooms: 'Classes',
    actions: 'Actions',
    assignStudent: 'Assign Student',
    assignStudents: 'Assign Students',
    classroom: 'Class',
    editClassroom: 'Edit Class',
    edit: 'Edit',
    updateClassroom: 'Update Class',
    details: 'Details',
    administration: 'Administration',
    student: 'Student',
    students: 'Students',
    startDate: 'Start Date',
    endDate: 'End Date',
    date: 'Date',
    enrollmentStartDate: 'Enrollment Start Date',
    enrollmentEndDate: 'Enrollment End Date',
    selectTeacher: 'Select Teacher',
    chooseCourse: 'Choose Course',
    studentCount: 'student Count',
    homeroom: 'Homeroom',
    homerooms: 'Homerooms',
    homeroomsMultiple: 'Homerooms (Multiple)',
    selectHomeroom: 'Select Homeroom',
    createHomeroom: 'Create Homeroom',
    addNewHomeroom: 'Add New Homeroom',
    editHomeroom: 'Edit Homeroom',
    updateHomeroom: 'Update Homeroom',
    required: 'Required',
    email: 'Email',
    password: 'Password',
    optional: 'Optional',
    firstName: 'First Name',
    lastName: 'Last Name',
    uploadFile: 'Upload a file',
    uploadPhoto: 'Upload Photo',
    selectCountry: 'Select Country',
    schoolCode: 'School Code',
    schoolName: 'School Name',
    schoolInformation: 'School Information',
    crop: 'Crop',
    cropImage: 'Crop Image',
    profilePicUpload: 'Profile Pic Upload',
    username: 'Username',
    parentProfile: 'Parent Profile',
    studentProfile: 'Student Profile',
    childProfile: 'Child Profile',
    confirmation: 'Confirmation',
    signUp: 'Sign Up',
    signUpCompletedMessage: 'All set, now choose courses!',
    next: 'Next',
    back: 'Back',
    goToDashboard: 'Go to Dashboard',    
    alreadyHaveAccountMessage: 'If you already have an account, <0>Login</0> to start your session.',
    noStoreItems: 'No Store Items',
    store: 'Store',
    image: 'Image',
    courseTitle: 'Course Title',
    courseDescription: 'Course Description',
    priceEach: 'Price Each',
    unassignedCredits: 'Unassigned Credits',
    role: 'Role',
    users: 'Users',
    usersNotFound: 'Users not found...',
    editUser: 'Edit User',
    updateUser: 'Update User',
    createUser: 'Create user',
    group: 'Group',
    gender: 'Gender',
    selectGender: 'Select Gender',
    male: 'Male',
    female: 'Female',
    phoneNumber: 'Phone Number',
    demoClassroom: 'Demo Class',
    selectCourse: 'Select Course',
    createDemoCourseClassroom: 'Create Demo Course Class',
    addNewClassroom: 'Add New Class',
    editDemoClassroomClassroom: 'Edit Demo Course Class',
    bulkAddHomerooms: 'Bulk Add homerooms',
    teachers: 'Teachers',
    assignStudentsToHomeroom: 'Assign students to homeroom',
    selectSchoolForStudents: 'Select a school for students list to be available.',
    accepted: 'Accepted',
    declined: 'Declined',
    pending: 'Pending',
    invitations: 'Invitations',
    invitationsNotFound: 'Invitations Not Found...',
    publisher: 'Publisher',
    status: 'Status',
    attemptDate: 'Attempt date',
    sendInvitation: 'Send Invitation',
    sendNewInvitation: 'Send new invitation',
    demoCourseInvitation: 'Demo Course Invitation',
    message: 'Message',
    createClassroom: 'Create Class',
    description: 'Description',
    accept: 'Accept',
    decline: 'Decline',
    userInvitedYouToDemoCourse: '<0>{{sender}}</0> has invited you to join their Demo launch of <1>{{course}}</1>',
    invitationDeclined: 'Invitation was declined.',
    invitationAccepted: 'Thanks for joining the Demo!',
    register: 'Register',
    login: 'Login',
    thanksForJoiningDemo: 'Thanks for joining the Demo',
    joinDemoLoginMessage: 'Please login if you already have an account otherwise you may register a new account.',
    joinDemoCorseIsFreeMessage: 'Note that you will not be charged for this course or asked for a credit card.',
    yourProfile: 'Your Profile',
    changePassword: 'Change Password',
    enterOldPassword: 'Enter Old Password',
    enterNewPassword: 'Enter New Password',
    confirmPassword: 'Confirm Password',
    cancel: 'Cancel',
    change: 'Change',
    uploadAvatar: 'Upload Avatar',
    uploadLogo: 'Upload Logo',
    saveChanges: 'Save Changes',
    none: 'None',
    birthday: 'Birthday',
    updateUserProfileNote: 'Submitting the following information is optional. Your profile is never shared.',
    summery: 'Summary',
    shippingAddress: 'Shipping Address',
    billing: 'Billing',
    schoolLogo: 'School Logo',
    save: 'Save',
    editProfile: 'Edit profile',
    search: 'Search',
    bulkAddStudents: 'Bulk Add Students',
    studentsNotFound: 'Students Not Found...',
    editStudent: 'Edit Student',
    updateStudent: 'Update Student',
    createStudent: 'Create Student',
    addNewStudent: 'Add New Student',
    bulkAddTeachers: 'Bulk Add Teachers',
    teachersNotFound: 'Teachers Not Found...',
    updateTeacher: 'Update Teacher',
    editTeacher: 'Edit Teacher',
    createTeacher: 'Create Teacher',
    addNewTeacher: 'Add New Teacher',    
    bulkAddInstructionHomeroom: 'Use the Homeroom names as how they are listed in this system',
    bulkAddInstructionCsv: 'Upload the csv file, with the following columns and separated by a comma \' , \'',
    bulkAddInstructionHeadings: 'Make sure that you place the column name headings in the first row',
    bulkAddInstructionColumnsOrder: 'Make sure that data is arranged in the order as the columns',
    bulkAddInstructionUserFields: 'You may leave other columns data blank except for the username and password. If they are empty, place a \'\' in their column position.',
    bulkAddInstructionDates: 'Dates are of the format YYYY-MM-DD.',
    bulkAddInstructionHomeroomFields: 'Do not leave any columns empty.',
    classRoomSchedule: '{{classroom}} -  Schedule',
    scheduleBy: 'Schedule By',
    unit: 'Unit',
    lesson: 'Lesson',
    frequency: 'Frequency',
    days: 'Days',
    weeks: 'Weeks',
    lessonTitle: 'Lesson Title',
    lessonDescription: 'Lesson Description',
    lessonAttempt: 'Lesson attempt',
    instructions: 'Instructions',
    type: 'Type',
    resource: 'Resource',
    count: 'Count',
    thumbnail: 'Thumbnail',
    courses: 'Courses',
    books: 'Books',
    bundles: 'Bundles',
    teachingAds: 'Teaching Aids',
    stationary: 'Stationary',
    studentReward: 'Student Reward',
    tutoringService: 'Tutoring Service',
    productsNotFound: 'Products Not Found...',
    viewCart: 'View Cart',
    checkout: 'Checkout',
    contactInformation: '{{title}} contact information',
    contactAddress: '{{title}} Address',
    addressLine1: 'Address Line 1',
    addressLine2: 'Address Line 2',
    city: 'City',
    stateOrProvince: 'State or Province',
    country: 'Country',
    postalOrZipCode: 'Postal or Zip Code',
    telephone: 'Telephone',
    paymentsOptions: 'Payments Options',
    yourInvoice: 'Your invoice <0>{{invoiceNo}}</0> Total <1>{{invoiceAmount}}</1>',
    shippingAndBilling: 'Shipping and Billing',
    payPal: 'PayPal',
    creditCard: 'Credit Card',
    check: 'Check',
    wireTransfer: 'Wire Transfer',
    cog: 'COD',
    creditCardNumber: 'Credit Card Number',
    holderName: 'Holder Name',
    cardType: 'Card Type',
    visa: 'Visa',
    masterCard: 'MasterCard',
    discover: 'Discover',
    amex: 'Amex',
    jcb: 'JCB',
    expDate: 'Exp. Date (YYYY/MM)',
    cvvCode: 'CVV Code',
    makePayment: 'Make Payment',
    sameBillingInformation: 'same billing information',
    shipping: 'Shipping',
    nextStep: 'Next Step',
    interactPayment: 'Interact payment',
    emailToDistributor: 'Email to {{email}}',
    wireTransferTo: 'Wire Transfer to',
    mailChequeTo: 'Mail Cheque to',
    downloadPdf: 'Download PDF',
    billTo: 'Bill to',
    shipTo: 'Ship to',
    courseName: 'Course Name',
    noUnassignedCredits: 'No Unassigned credits...',
    selectStudent: 'Select Student',
    ourAccount: 'Our Account',
    shoppingCart: 'Shopping Cart',
    product: 'Product',
    subject: 'Subject',
    age: 'Target Age',
    info: 'Info',
    noInfo: 'No Info...',
    buy: 'Buy',
    total: 'Total',
    authorized: 'Authorized',
    created: 'Created',
    transactions: 'Transactions',
    title: 'Title',
    quantity: 'Quantity',
    price: 'Price',
    noTransactions: 'No transactions...',
    openInvoices: 'Open Invoices',
    open: 'Open',
    storeIsEmpty: 'Store is empty',
    more: 'More',
    similar: 'Similar',
    elementaryGrade1: 'Elementary Grade 1',
    kindyStarter: 'Kindy Starter',
    kindyAdvanced: 'Kindy Advanced',
    elementary1to3: 'Elementary 1-3',
    elementary4to6: 'Elementary 4-6',
    juniorHighSchool: 'Junior High School',
    highSchool: 'High School',
    adult: 'Adult',
    senior: 'Senior',
    newest: 'Newest',
    all: 'All',
    flex: 'Flex',
    fineArts: 'Fine Arts',
    safety: 'Safety',
    language: 'Language',
    englishForKids: 'English For Kids',
    rating: 'Rating',    
    targetAge: 'Target  Age',
    sortBy: 'Sort By',
    createNow: 'Create now',
    maxStudents: 'Max Students',
    rolloverDayTime: 'Rollover day time',
    autoClassroomsNotFound: 'Auto Classes Not Found...',
    autoClass: 'Auto Class',
    rolloverDay: 'Rollover day',
    monday: 'Monday',
    tuesday: 'Tuesday',
    wednesday: 'Wednesday',
    thursday: 'Thursday',
    friday: 'Friday',
    saturday: 'Saturday',
    sunday: 'Sunday',
    rolloverDayNumber: 'Rollover day number',
    autoClassroomSimilarityRegularCreation: 'Auto Class Similarity Regular Creation',
    startAutoClassroom: 'Start Auto Class',
    durationInDays: 'Duration in days',
    weekly: 'Weekly',
    monthly: 'Monthly',
    signIn: 'Sign In',
    forgetPassword: 'Forget Password',
    rememberMe: 'Remember me',
    doNotHaveAccountYet: 'Don\'t have an account yet?',
    request: 'Request',
    forgotPassword: 'Forgot Password',
    enterEmailToResetPassword: 'Enter your email to reset your password',
    continueShopping: 'Continue shopping',
    shippingAndTax: 'Shipping & Tax',
    noOpenInvoices: 'No Open invoices',
    deleteItemFromCartConfirmation: 'Are you sure you want to delete {{item}} from the shopping cart?',
    itemsCount: '{{count}} items',
    delete: 'Delete',
    howToMovies: 'How-to Movies',
    reportsSnapshot: 'Reports Snapshot',
    roster: 'Roster',
    admins: 'Admins',
    quickLinks: 'Quick Links',
    howTo: 'How-To',
    movies: 'Movies',
    studentApp: 'Student App',
    download: 'Download',
    studentsOnline: 'Students Online',
    oneDay: '1 day',
    oneWeek: '7 days',
    oneMonth: '1 month',
    oneYear: '1 year',
    chooseDate: 'Choose date',
    selectDay: 'Select day',
    selectWeek: 'Select week',
    selectMonth: 'Select month',
    selectYear: 'Select year',
    chooseStartDate: 'Choose start date',
    schoolAverage: 'School Average',
    performance: 'Performance',
    progress: 'Progress',
    error: 'Error',
    passRate: 'Pass Rate',
    complete: 'complete',
    inProgress: 'in progress',
    notStarted: 'not started',
    correct: 'correct',
    incorrect: 'incorrect',
    about: 'About',
    myCourses: 'My Courses',
    remarks: 'Remarks',
    reports: 'Reports',
    unitLesson: 'Unit/Lesson',
    score: 'Score',
    percent: 'Percent',
    passFail: 'Pass/Fail',
    comments: 'Comments',
    passesAttempts: 'Passes/Attempts',
    pass: 'Pass',
    fail: 'Fail',
    detailedData: 'Detailed Data',    
    dashboard: 'Dashboard',
    studentsAndStaff: 'Students & Staff',
    learningAreas: 'Learning Areas',
    classInvitations: 'Class Invitations',
    professionalTrainings: 'Professional Trainings',
    sharing: 'Sharing',
    inbox: 'Inbox',
    labels: 'Labels',
    sent: 'Sent',
    compose: 'Compose',
    drafts: 'Drafts',
    chat: 'Chat',
    teachingAids: 'Teaching Aids',
    studentRewards: 'Student Rewards',
    tutoringServices: 'Tutoring Services',
    accounts: 'Accounts',
    historyTransactions: 'History (Transactions)',
    calendar: 'Calendar',
    myOrders: 'My Orders',
    sCap: 'S-CAP',
    averageGrade: 'Average Grade',
    noStudents: 'No Students...',
    noClassrooms: 'No Classes...',
    noHomerooms: 'No Homerooms...',
    paypal: 'PayPal',
    creditcard: 'Credit Card',
    completed: 'Completed',
    personalInformation: 'Personal Information',
    setupYourPrincipalProfile: 'Setup Your Principal Profile',
    myLearners: 'My Learners',
    reportProfile: 'Report/Profile',
    logInDifferentUser: 'Log In different user',
    logIn: 'Log in',
    reset: 'Reset',
    areYouSure: 'Are you sure want to remove this item?',
    downloadCsvMessage: 'You may download this .csv template to use it as pattern for your csv file. Kindly make sure that you have a valid .csv format before uploading to the system.',    
    selectCsvFile: 'Select CSV file',    
    inserted: 'Inserted',
    failed: 'Failed',
    results: 'Results',
    youHaveNoNewMessages: 'You have no new messages',
    seeAll: 'See All',
    subscriptions: 'Subscriptions',
    subscription: 'Subscription',
    courseAtTime: '<0>{{courses}}</0> course at a time',
    courseAnyCourseSwitchAnyTime: 'Choose any course; switch any time',
    courseAnyCoursesSwitchAnyTime: 'Choose any <0>{{courses}}</0> courses; switch any time',
    usersMax: '<0>{{students}}</0> users max',
    userMax: '<0>{{students}}</0> user max',
    annualBonus: 'Annual bonus',
    perMonth: 'Per Month',
    perYear: 'Per Year',
    month: 'Month',
    year: 'Year',    
    courseSubscriptionOptions: 'Course Subscription Options',
    subscriptionsNotFound: 'Subscriptions Not Found...',
    getThis: 'Get This',
    mySubscriptions: 'My Subscriptions',
    allowedCourses: 'Courses Available',
    assignedCourses: 'Courses Assigned',
    expirationDate: 'Exp. Date',
    isActive: 'Active',
    assignedStudents: 'Assigned Students',
    assignDate: 'Assign Date',
    areYouSureWantToCancelSubscription: 'Are you sure want to cancel this subscription?',
    areYouSureWantToCancelThisCourse: 'Are you sure want to cancel this course?',
    noSubscriptions: 'No Subscriptions...',
    noSubscribedUsers: 'No Subscribed Students...',
    buySubscription: 'Buy Subscription',
    yes: 'Yes',
    no: 'No',
    switchPlan: 'Switch Plan',
    continue: 'Continue',
    youBoughtSubscription: 'Congratulations! You have successfully purchased a new subscription.',
    viewMySubscriptions: 'View My Subscriptions',
    subscriptionDetails: 'Subscription Details',
    yourPlan: 'Your Plan',
    selectYourAccountType: 'Select your account type',
    parent: 'Parent',
    principal: 'Principal',
    paymentsNotFound: 'No Subscription Payments...',
    subscriptionPayments: 'Subscription Payments',
    transactionCode: 'Transaction #',
    yourShippingAndBillingInfoSaved: 'Your shipping and billing info has been saved',
    creatingRequest: 'Creating {{paymentType}} request...',
    failPaymentMessage: 'Unfortunately your payment has failed',
    canceledPaymentMessage: 'Your payment was canceled',
    pendingPaymentMessage: 'Your payment is now pending',
    loginToSeeYourTransactions: 'To view your transaction please log in',
    orderDetails: 'Order details',
    messages: 'Messages',
    typeHere: 'Type here...',
    noMessagesYet: 'No messages yet',
    searchUser: 'Search...',    
    conversations: 'Conversations',
    templatesNotFound: 'No S-Cap Templates...',
    questions: 'Questions',
    question: 'Question',
    addNewQuestion: 'Add New Question',
    buildTemplate: 'Build Template',
    noAnyQuestions: 'No questions were added...',
    enterQuestionText: 'Please enter question text here',
    saveTemplate: 'Save Template',
    pleaseAddAnyQuestions: 'Please add some questions to your S-CAP template...',
    not_published: 'Not Published',
    templateDetails: 'Template Details',
    updateScapTemplate: 'Update S-Cap Template',
    public: 'Public',
    assignTeachers: 'Assign Teachers',
    classroomNotPaid: 'Class is not paid',
    classroomPaid: 'Class is paid',
    sCapResults: 'S-Cap Results',    
    fillScapTemplate: 'S-Cap Template',
    summary: 'Summary',
    shareScapToTeachers: 'Share S-Cap with Teachers',
    viewScapResults: 'View S-Cap Results',
    scapResultsNotFound: 'No S-Cap Results found...',
    notes: 'Notes',
    saveAndPublish: 'Save and Publish',
    saveAsDraft: 'Save as Draft',
    draft: 'Draft',
    googlepay: 'Google Pay',
    applepay: 'Apple Pay',
    Superadministrator: 'Super Admin',
    schools: 'Schools',
    newMessage: 'New Message',
    mail: 'Mail',
    alert: 'Alert!',    
    recipient: 'Recipient',
    recipients: 'Recipients',
    entireSchool: 'Entire School',
    composeMessage: 'Compose Message',
    studentsAndParents: 'Students & Parents',
    studentRole: 'Students',
    parentsRole: 'Parents',
    teacherRole: 'Teachers',
    principalRole: 'Principals',
    administratorRole: 'School Administrators',
    superadministratorRole: 'Super Administrators',
    superintendentRole: 'Superintendents',
    selectHomerooms: 'Select Homerooms',
    selectClassrooms: 'Select Classes',
    selectRoles: 'Select Roles',
    sendMessage: 'Send Message',
    selectAll: 'Select All',
    sentDate: 'Sent At',
    sentMessages: 'Sent Messages',
    inboxMessages: 'Inbox Messages',
    draftMessages: 'Draft Messages',
    viewMessage: 'View Message',
    messagesNotFound: 'No messages found...',
    reply: 'Reply',
    replyMessage: 'Reply Message',
    new: 'New',
    from: 'From',
    to: 'To',
    schoolManagement: 'School Management',
    noDemoCoursesFound: 'No Demo Courses found...',
    code: 'Code',
    areYouSureWantToArchiveClassroom: 'Are you sure want to archive this Class?',
    invitationExpired: 'Sorry, this invitation has expired...',
    expired: 'Expired',
    enterParentUsernameOrEmail: 'Parent username or email',
    linkToParent: 'Link to Parent\'s Account',
    createParentAccount: 'Create Parent\'s Account',
    sendRequest: 'Send Request',
    youSentRequestToParent: 'You successfully sent request to this user. You will be notified if the request is accepted...',
    myParents: 'My Parents',
    requestSentDate: 'Request sent at',
    parentDeclinedYourRequest: 'Your request has been declined...',
    rejectRequest: 'Reject Request',
    parentsNotFound: 'No parents found...',
    connectionsNotFound: 'No connections found...',    
    giftsNotFound: 'No gifts found...',   
    addParent: 'Add parent',    
    declinedAt: 'Declined at',
    noData: 'No data found...',
    sentRequest: 'Sent Request',
    myConnections: 'My Connections',
    connectionRequest: 'Connection Request',
    enterUsernameOrEmail: 'Enter Username Or Email',
    connectionRequestWaitingForYourApproval: 'This connection request is awaiting your approval...',
    connectionRequestSent: 'You successfully sent connection request to this user. You will be notified if the request is accepted....',
    connectionRequestDeclined: 'This connection requst has been declined by the recipient...',
    areYouSureWantToDeclineThisRequest: 'Are you sure want to decline this connection?',
    areYouSureWantToDeclineThisGift: 'Are you sure want to decline this gift?',
    connectionDetails: 'Connection Details',
    giftNotification: '{{user}} has sent you a gift.  To accept, please click the link below.',
    giftCourseCreditNotification: 'NOTE: {{user}} has send your a BZabc course credit.  To accept, please click below.',
    waiting: 'Waiting',
    giftSubscription: 'Gift Subscription',
    makeGift: 'Make Gift',
    giftToPersone: 'Select Person',
    selectPersone: 'Select Person',
    activeSubscriptions: 'Active Subscriptions',
    expiredSubscriptions: 'Expired Subscriptions',
    filterSubscriptions: 'Filter Subscriptions',
    purchasedFor: 'Purchased For',
    self: 'Self',
    giftForYou: 'You',
    deleteMobileSubscription: 'This subscription was created via mobile application, Apple iTunes or Google Play. You can cancel this one using BZabc Parents mobile application only.',
    giftFrom: 'This subscription is purchased for you by {{user}}.',
    giftCourseFrom: 'This course credit is purchased for you by {{user}}.',    
    giftCourseCredit: 'Gift Course Credit',
    acceptInviteAndCreateAccount: 'Accept Invitation from {{ user }}',
    createAccount: 'Create Account',
    inviteAlreadyRegisteredUser: 'Invite Already Registered User',
    inviteNewUser: 'Invite New User',
    invite: 'Invite',
    selectFromMyConnections: 'Connected users',
    inviteNewPersone: 'Invite New Persone',
    pleaseSelectConnectedUserOrEnterEmailAddress: 'Please select connected user from the list or or enter email address to invite new user.',
    receiver: 'Receiver',
    myGifts: 'My Gifts',
    sender: 'Sender',
    lessonInformation: 'Lesson Information',
    perfomance: 'Perfomance',
    unassignedCourses: 'Unassigned Courses',
    manageCourses: 'Manage Courses',
    upload: 'Upload',
    invoice: 'Invoice',
    yourCartIsEmpty: 'Your shopping cart is empty...',
    due: 'Due',
    paid: 'Paid',
    filterTransactions: 'Filter Transactions',
    storeSlider1: 'Save Big With course bundles',
    storeSlider2: 'BZabc:Buy the entire series!',
    storeSlider3: 'New Movies to Rent',
    youHaveUnassignedCourseCredits: 'You have {{ count }} unassigned course credit(s)',
    assignAllowedCourseCreditsToSelectedStudents: 'Assign allowed Course credits to Class students',
    viewInvitation: 'View Invitation',
    sendGift: 'Send a Gift',
    yourName: 'Your Name',
    yourEmail: 'Your Email',
    recipientName: 'Recipient Name',
    recipientEmail: 'Recipient Email',
    storePayments: 'Store Payments',
    items: 'Items',
    period: 'Period',
    manage: 'Manage',
    manageSubscriptions: 'Manage Subscriptions',
    deleteLearnerConfirmation: 'Are you sure want to remove this learner?',
    downloadReportInPdf: 'Download Report In PDF',
    enrollments: 'Enrollments',
    autoRenewal: 'Auto Renewal',
    showDetails: 'Show Details',
    groups: 'Groups',
    recipientGroups: 'Recipients Groups',
    userRolesInGroups: 'User Roles In Groups',
    selectStudents: 'Select Students',
    messageGroupsNotFound: 'No groups found...',
    newGroup: 'New Group',
    updateGroup: 'Update Group',
    passwordResetMessage: 'A restore password link has been send to your email',
    setNewPassword: 'Set New Password',
    usernameOrEmail: 'Username Or Email',
    expires: 'Expires',
    alerts: 'Alerts',
    announcements: 'Announcements',
    assignments: 'Assignments',
    chats: 'Chats',
    announcement: 'Announcement',
    assignment: 'Assignment',
    newAlert: 'New Alert',
    newAnnouncement: 'New Announcement',
    newAssignment: 'New Assignment',
    editAlert: 'Edit Alert',
    editAnnouncement: 'Edit Announcement',
    editAssignment: 'Edit Assignment',    
    loading: 'Loading...',
    me: 'Me',
    parents: 'Parents',
    addNewParent: 'Add New Parent',
    updateParent: 'Update Parent',
    createParent: 'Create Parent',
    editParent: 'Edit Parent',
    chatName: 'Chat Name',
    newChat: 'New Chat',
    participants: 'Participants',
    visibleForParticipants: 'Visible for participants only',
    groupChatsNotFound: 'Group chats not found...',
    privateChatsNotFound: 'Private chats not found...',
    searchGroups: 'Search Groups',
    searchUsers: 'Search Users',
    youNotHaveStudents: 'You don\'t have any students. Please use "My Learners" tool on the dashboard to register new students.',
    messageRemoved: 'Removed by sender...',
    editMessage: 'Edit Message:',
    viewReport: 'View Report',
    ok: 'Ok',
    skip: 'Skip',
    received: 'Received',
    Basic: 'Basic',
    Multi: 'Multi',
    Family: 'Family',
    freeWorkbook: '1 free workbook',
    freeWorkbooks: '{{number}} free workbooks',
    subscriptionIsActive: 'Subscription is active. Next recurring payment will be charged at {{date}}.',
    subscriptionWillBeClosed: 'Recurring payments are disabled. Subscription will be closed at {{date}}.',
    subscriptionIsClosed: 'Subscription is expired and closed.'
}
