    ...
    return rows.map(function (item, i) {
      ...
      return (
        <Row index={i} key={i}>
          ...
          <Td width='132px'>
            <input
              disabled={item.isInvoice}
              type="number"
              onChange={(e) => {_self._changeItemCount(i,e)}}
              value={item.count}
              className="form-control productQuantity m-input m-input--solid"
              style={{height:"50px"}}/>
          </Td>
          <Td width='132px'>
          ...
          <Td width="50px">
            {! item.isInvoice &&
              ...
              }} className="btn btn-danger m-btn m-btn--icon btn-lg m-btn--icon-only m-btn--pill">
