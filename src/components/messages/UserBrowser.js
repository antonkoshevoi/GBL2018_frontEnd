...
class UserBrowser extends Component {
    ...
    _renderUsers () {
        const { users, loading, thread, t } = this.props;
        const activeUserName = (thread !== undefined) ? thread.get('user').get('username') : '';

        if (!loading && users.size === 0) {
            return (
                <div>
                    <span className="m-list-search__result-item-text">{t('usersNotFound')}</span>
                </div>
            );
        }
            ...
            return (
                <div className={`m-widget4__item pointer ${activeUserName === user.get('username') && 'active'}`} key={key}
                    ...
                    <div className="m-widget4__info">
                          ...
                          <span className="m-widget4__sub"></span>
                    </div>
                </div>       
            );
        });
  ...
  render() {
    const { loading, onSearch, t } = this.props;
...
export default translate('translations')(UserBrowser);