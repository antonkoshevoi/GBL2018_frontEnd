        ...
        return records.map((record, i) => (      
            <Row index={i} key={i}>
              ...
              <Td className="actions">
                ...
                :
                    <button title={t('giftCourseCredit')} className='btn btn-warning m-btn--icon-only' onClick={() => { this._openGiftDialog(record) }} >
                        <i className='la la-gift'></i>
                    </button>
                }          
                <button title={t('assignStudent')} className='btn btn-warning m-btn--icon-only ml-2' onClick={() => { this._openAssignDialog(record) }} >
